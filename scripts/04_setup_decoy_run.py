"""
Step 04 — Set up decoy run environment.
...
Usage:
    python scripts/04_setup_decoy_run.py
    python scripts/04_setup_decoy_run.py --positives path/to/other.csv --version v1.0.0
"""
...
def split_positives(positives_path: str) -> int:
    ...
    for split_idx, group in df.groupby("split"):
        ...
        group[["canonical_smiles"]].rename(columns={"canonical_smiles": "smiles"}).to_csv(out_path, index=False)
...
def print_sbatch_command(n_splits: int) -> None:
    max_idx = n_splits - 1
    script_path = os.path.join(ROOT, "05_run_decoys.sh")
    print(
