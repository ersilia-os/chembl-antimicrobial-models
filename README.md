...
| 03 | `scripts/03_setup_decoy_run.py` | Splits positives into batches, builds the `eos3e6s` Apptainer SIF image via `ersilia_apptainer create` (accepts `--version`, default `v1.0.0`), and writes the SLURM submission script; requires `envs/camm` from the Setup step |
| 04 | `scripts/04_run_decoys.sh` | SLURM array job (generated by step 03, submit with `sbatch scripts/04_run_decoys.sh`) that runs `eos3e6s` on each input split via `ersilia_apptainer` and writes results to `output/results/04_decoys/` |
